...
### Phase 1: Quick Wins with Option 3 (Week 1)

**Immediate implementation of custom resolvers:**

```java
// Week 1: Get polymorphic operators working
registry.registerCustom("in", new InOperatorResolver());
registry.registerCustom("union", new UnionOperatorResolver());
registry.registerCustom("iif", new IifResolver());
registry.registerCustom("contains", new ContainsOperatorResolver());
...
- Validates common type computation approach
- Builds confidence in design
- Zero risk to existing code

### Phase 2: Commit to Option 2 Migration (Weeks 2-4)

**Full migration to element-first pure:**

```java
// Week 2: Foundation
- Add Cardinality enum
- Add TypeVariable and Bottom types
- Add getCardinality() to IRNode interface

// Week 3: Type system migration
- Remove CollectionType from hierarchy
- Update all IR node constructors
- Implement TypeAdapter with simplified rules
- Implement TypeBindings for unification

// Week 4: Signature migration
- Convert all signatures to new format
- Update Analyzer resolution logic
- Migrate custom resolvers to declarative signatures
- Comprehensive testing
```
...
```java
// Week 5+
- Add constraint predicates for type sets
- Optimize common type computation (caching)
- Enhanced error messages with type information
- Documentation and examples
- Performance benchmarking
```

### Alternative: Stay with Option 3 if...

**When NOT to migrate to Option 2:**

1. CollectionType removal causes too many downstream issues
2. Team prefers programmatic over declarative signatures
3. Only need polymorphic signatures for 3-4 operators
4. Higher priority work emerges
5. System is stable and working well

In this case, **improve Option 3** instead:
- Extract common type logic to shared utility
- Add more sophisticated type inference
- Consider adding limited TypeVariable support to SimpleSignature
- Eventually migrate to Option 1 instead of Option 2
...
All three options are viable, with different tradeoffs:

- **Option 1** balances migration cost with declarative signatures
- **Option 2** provides the cleanest long-term design aligned with spec
- **Option 3** offers immediate results with minimal risk

**Recommended approach**: Start with **Option 3** for quick wins, then migrate to **Option 2** for long-term maintainability and spec alignment. If Option 2 migration proves too costly, **Option 1** is a solid middle ground.

The key insight is that cardinality should be separate from semantic types, whether as a Shape abstraction (Option 1) or as pure metadata (Option 2). Option 3 buys time to validate this approach before committing to the structural changes.
...
1. **Immediate** (this week):
   - Implement Option 3 custom resolvers for `in`, `union`, `iif`
   - Validate common type computation logic
   - Test with real FHIRPath expressions

2. **Short-term** (next sprint):
   - Decide between Option 1 and Option 2 based on team feedback
   - Plan migration timeline and breaking change communication
   - Begin implementation of chosen option

3. **Long-term** (next quarter):
   - Complete migration to chosen option
   - Comprehensive testing and documentation
   - Performance optimization and error message improvements