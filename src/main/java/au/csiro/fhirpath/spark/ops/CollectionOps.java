 ...
 * indexer).
 *
 * <p>Uses {@link au.csiro.fhirpath.spark.SparkDefs#collectionUnary collectionUnary} for simple
 * cardinality-dispatched operations, and direct {@code ctx.collectionArg(0).apply()} for operations
 * that need additional arguments.
 */
  ...
  private static Column generateSkip(@Nonnull final SparkOpContext ctx) {
    ...
    return ctx.collectionArg(0)
        .apply(
            ...
            c -> when(c.isNull().or(n.gt(lit(0))), lit(null)).otherwise(c));
  }
  ...
  private static Column generateTake(@Nonnull final SparkOpContext ctx) {
    ...
    return ctx.collectionArg(0)
        .apply(
            ...
            c -> when(c.isNull().or(n.leq(lit(0))), lit(null)).otherwise(c));
  }
