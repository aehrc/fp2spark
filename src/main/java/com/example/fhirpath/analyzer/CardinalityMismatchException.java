 ...
 *       If there is more than one item, the evaluator will signal an error.
 *   <li>Section 3196-3197: Comparison operators require collections with single values.
 *       The evaluator will throw an error if either collection has more than one item.
 *   <li>Section 768-783: Singleton evaluation rules define when collections can be
 *       implicitly converted to single values.
 * </ul>
 *
...
public class CardinalityMismatchException extends AnalysisException {
    ...
    @Nonnull
    private final Cardinality actual;
    @Nonnull
    ...
    private static String formatMessage(
        ...
        String cardinalityName = expected == Cardinality.SINGLE ? "single" : "multiple";
        String paramName = parameterIndex == 0 ? "left operand" : "right operand";

        return String.format(
                "Operation '%s' requires %s to have %s cardinality, but got %s cardinality.\n" +
                "  FHIRPath spec: Math and comparison operators require each operand to be a %s element.",
                operationName,
                paramName,
                expected,
                actual,
