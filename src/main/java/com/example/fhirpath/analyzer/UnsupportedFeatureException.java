 ...
 *
 * <p>This is used for valid FHIRPath constructs that are recognized by the parser
 * but not yet supported by the analyzer or code generator.
 *
 * <p>Examples:
 * <ul>
...
public class UnsupportedFeatureException extends AnalysisException {
    ...
    private static String formatMessage(String featureName) {
        return String.format("FHIRPath feature '%s' is not yet implemented", featureName);
    }

    private static String formatMessageWithDetail(String featureName, String detail) {
        return String.format(
                "FHIRPath feature '%s' is not yet implemented: %s",
                featureName,
                detail
        );
    }
