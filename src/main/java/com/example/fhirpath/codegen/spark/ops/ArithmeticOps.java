...
public final class ArithmeticOps {
    ...
    public static void register(SparkOperationRegistry registry) {
        registry.register("add", (args, nodes, type, gen) ->
                switch ((PrimitiveType) type) {
                    ...
                    default -> throw new IllegalArgumentException(
                            "Unsupported result type for add: " + type);
                });

        registry.register("sub", (args, nodes, type, gen) ->
                switch ((PrimitiveType) type) {
                    case INTEGER, DECIMAL -> args.get(0).minus(args.get(1));
                    default -> throw new IllegalArgumentException(
                            "Unsupported result type for sub: " + type);
                });

        registry.register("multiply", (args, nodes, type, gen) ->
                switch ((PrimitiveType) type) {
        ...
        registry.register("divide", (args, nodes, type, gen) ->
                switch ((PrimitiveType) type) {
                    case INTEGER, DECIMAL -> args.get(0).divide(args.get(1));
                    default -> throw new IllegalArgumentException(
                            "Unsupported result type for divide: " + type);
                });

        registry.register("mod", (args, nodes, type, gen) ->
                args.get(0).mod(args.get(1)));
    }
}
