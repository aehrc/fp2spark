 ...
 * <ul>
 *   <li>Operation registry lookup ({@link #getSignatures})</li>
 *   <li>Overload resolution ({@link #resolveCall})</li>
 *   <li>Complete resolution for simple cases ({@link #resolveBinaryOperator})</li>
 * </ul>
 *
 * <p>Infrastructure operations (getValue, equals, union) are handled separately by
 * {@link InfrastructureOperationHandler} and do not use this resolution path.
 *
 * <p><b>Design Note:</b> Function call resolution requires interleaved argument analysis
 * (lambdas need signature information before analysis), so this facade provides both
 * high-level ({@link #resolveBinaryOperator}) and low-level ({@link #getSignatures},
 * {@link #resolveCall}) methods to support different resolution patterns.
 *
 * <p>Usage:
 * <pre>{@code
...
public final class OperationResolver {
     ...
     *
     * <p>This is a low-level method for cases where signature information is needed
     * before argument analysis (e.g., lambda parameter detection).
     *
     ...
     *
     * <p>This is a low-level method that performs overload resolution given a list
     * of candidate signatures and analyzed arguments.
     *
     ...
     *
     * <p>Normalizes the operator symbol to canonical name (e.g., "+" → "add", "=" → "equals"),
     * then performs registry lookup and overload resolution.
     *
     * <p><b>Note:</b> Infrastructure operations (equals, union) should be handled separately
     * before calling this method, as they have special resolution semantics.
     *
