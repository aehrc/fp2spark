...
/**
 * Exception thrown when no matching function/operator overload can be found
 * for the given argument types.
 *
 * <p>This occurs when calling a function or operator with arguments whose types
 * don't match any registered signature after type adaptation attempts.
 *
 * <p>Example: Calling {@code add(STRING, INTEGER)} when only
 * {@code add(INTEGER, INTEGER)} and {@code add(DECIMAL, DECIMAL)} are defined.
 */
public class OverloadResolutionException extends AnalysisException {
    ...
    public OverloadResolutionException(
            ...
            @Nonnull List<Type> argumentTypes,
    ...
    private static String formatMessage(String operationName, List<Type> argumentTypes) {
